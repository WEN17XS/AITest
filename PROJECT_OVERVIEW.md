...
README.md                启动说明
DESIGN.md                前端视觉风格基准，当前采用 Supabase 风格规范
```
...
根目录 `DESIGN.md` 是前端长期视觉风格基准，来源于 `awesome-design-md` 的 Supabase 风格规范。

当前前端样式已按该规范重写，整体约定为：

- 以白色和近黑为主体，浅灰背景、细边框和低阴影承载工作台层级。
- 绿色 `#3ecf8e` / `#24b47e` 只用于主操作、焦点和关键强调。
- 卡片、面板、导航、表单、状态标签保持控制台式信息密度，不做营销页式大面积装饰。
- 代码、日志、报告正文使用深色代码块，便于和表单/列表区分。
- 后续新增或重构前端页面时，优先沿用 `DESIGN.md` 和 `frontend/src/styles.css` 中的 CSS 变量、圆角、边框、按钮、表单、状态标签和布局规则。
...
- 前端样式已导入并采用根目录 `DESIGN.md` 的 Supabase 风格规范，`npm run build` 通过，登录页已在浏览器中验证可正常渲染。
