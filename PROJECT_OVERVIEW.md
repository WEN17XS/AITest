...
第二阶段执行器架构已设计，详见 `docs/executor-architecture.md`。长期方向是将执行层拆分为运行编排层、执行器注册表、具体执行器和结果/附件持久化；`execution_service.py` 保留为编排入口，当前模拟执行器后续迁移为 `MockExecutor`，并继续作为闭环兜底能力。
...
1. 先抽出执行器基础接口、注册表和 `MockExecutor`，保持当前运行行为兼容。
2. 给测试运行增加执行器类型、环境引用、执行器配置和错误信息字段。
3. 增加测试运行附件模型，用于承载截图、视频、trace、日志和结构化报告。
4. 接入 Playwright Web UI 执行器。
5. 接入 pytest/API 执行器。
6. 增加失败重跑、flaky 用例识别和失败归因 Agent。
